...
USE `DB_SOCIAL_PORTAL`$$
DROP TRIGGER IF EXISTS `DB_SOCIAL_PORTAL`.`INVITATION_T_AFTER_UPDATE` $$
USE `DB_SOCIAL_PORTAL`$$
CREATE DEFINER = CURRENT_USER TRIGGER `DB_SOCIAL_PORTAL`.`INVITATION_T_AFTER_UPDATE` AFTER UPDATE ON `INVITATION_T` FOR EACH ROW
BEGIN
	IF NEW.`ACCEPTED` = TRUE THEN 
    BEGIN
		DELETE FROM `DB_SOCIAL_PORTAL`.`INVITATION_NOTIFICATION_T` 
        WHERE `DB_SOCIAL_PORTAL`.`INVITATION_NOTIFICATION_T`.`FOR_USER_ID` = NEW.`TO_USER_ID`
        AND `DB_SOCIAL_PORTAL`.`INVITATION_NOTIFICATION_T`.`INVITATION_ID` = `NEW`.`INVITATION_ID`;
	END; END IF;
END$$
